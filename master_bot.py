...
async def shopify_check(ctx, url):
    check = shopify.shopify_check(url)
    if check == True:
        embed = discord.Embed(title=":white_check_mark: ***THE URL*** __***IS***__ ***A SHOPIFY WEBSITE!***".format(url), url=url, color=0xffffff)
        embed.set_footer(icon_url=icon_img, text=footer_text)
        await client.send_message(ctx.message.channel, embed=embed)
        #await client.send_message(ctx.message.channel, ":white_check_mark: [THE URL]({}) __**IS**__ A SHOPIFY WEBSITE!".format(url))
    elif check == False:
        embed = discord.Embed(title=":no_entry_sign: ***THE URL*** __***IS NOT***__ ***A SHOPIFY WEBSITE!***".format(url), url=url, color=0xffffff)
        embed.set_footer(icon_url=icon_img, text=footer_text)
        await client.send_message(ctx.message.channel, embed=embed)
