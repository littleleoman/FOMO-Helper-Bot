...
def find_auth_token(website):
    ...
    return [token['value'] for token in soup.findAll('input',
...
def shopify_gen(website, email):
    ...
    submit_account = session.post(account_url, data=data,
                                  headers=headers)
    ...
    if submit_account.url == challenge_url:
        ...
        recaptcha_params = {'page_url': captcha_url,
                            'sitekey': sitekey, 'type': 1}  # optional, 1 - normal recaptcha, 2 - invisible recaptcha, 3 - v3 recaptcha, default: 1
        ...
        submit_captcha = session.post(account_url, data=captcha_data,
...
def tiny(url):
    URL = 'https://tinyurl.com/create.php?source=indexpage&url=' + url \
